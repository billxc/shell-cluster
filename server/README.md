...
Connect to `ws://<host>:<port>/raw` from any xterm.js client, or use the API at `http://<host>:9000/api/peers` to discover peers.
...
- **SerializeAddon** for perfect state replay on reconnect (no raw scrollback buffer)
